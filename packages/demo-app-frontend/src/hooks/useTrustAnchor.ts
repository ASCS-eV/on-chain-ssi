  ...
  useEffect(() => {
    async function fetchHistory() {
      ...
      const decodedProposals = proposalLogs.map((log: unknown) => {
        const { id, data, requiresUnanimity } = (
          log as { args: { id: bigint; data: `0x${string}`; requiresUnanimity: boolean } }
        ).args
        ...
        try {
          ...
          switch (funcName) {
            ...
            case '_executeChangeOwner':
              description = `Register Company: ${(decodedArgs[0] as string).slice(0, 6)}...`
              break
            default:
