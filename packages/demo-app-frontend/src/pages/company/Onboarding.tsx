...
import { ArrowRight, CheckCircle2, Building, Loader2, ShieldCheck, Clock, RefreshCw } from 'lucide-react'
...
export function CompanyOnboardingPage() {
  ...
  const { writeContract, data: hash, isPending } = useWriteContract({
    mutation: {
        onError: (err) => toast.error("Delegation Failed", { description: err.message.split('\n')[0] })
    }
  })
  ...
  const { data: identityOwner, isLoading: isOwnerLoading, refetch: refetchOwner } = useReadContract({
  ...
  const { data: isCompanyAdmin, isLoading: isAdminLoading, refetch: refetchAdmin } = useReadContract({
  ...
  // DERIVE STATE
  const isManaged = identityOwner && TRUST_ANCHOR_ADDRESS && 
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500 mt-2">Secure your digital identity within the Trust Anchor ecosystem.</p>
      </div>
              ...
              <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
                  <div className="flex items-center gap-4 mb-4">
                      ...
                      <div className="overflow-hidden">
                          ...
                          <p className="text-xs font-mono text-slate-500 truncate">{address || "Not Connected"}</p>
                      </div>
                  </div>
                  ...
                  <div className="space-y-3">
                      ...
                      <div className="flex justify-between text-sm py-2 border-b border-slate-200">
                          ...
                          {isAdminLoading ? (
                              ...
                              <span className={`font-bold ${isCompanyAdmin ? 'text-green-600' : 'text-slate-400'}`}>
